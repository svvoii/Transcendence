...
// The AI Object (the two paddles that move up and down)
var Ai = {
	new: function (side) {
		return {
			width: 18,
...
var Game = {
	...
	// Update all objects (move the player, ball, AI, increment the score, etc.)
	update: function () {
		if (!this.over) {
			...
			// Move the ball in intended direction based on moveY and moveX values
			if (this.ball.moveY === DIRECTION.UP) this.ball.y -= (this.ball.speed / 1.5);
			else if (this.ball.moveY === DIRECTION.DOWN) this.ball.y += (this.ball.speed / 1.5);
			if (this.ball.moveX === DIRECTION.LEFT) this.ball.x -= this.ball.speed;
			else if (this.ball.moveX === DIRECTION.RIGHT) this.ball.x += this.ball.speed;

			// Handle AI movement UP and DOWN
		...
		if (this.player.score === rounds[this.round]) {
			// Check to see if there are any more rounds/levels left and display the victory screen if the game is over.
			if (!rounds[this.round + 1]) {
				this.over = true;
				setTimeout(function () { Pong.endGameMenu('Winner!'); }, 1000);
			} else {
				// If there is another round, reset the turn and reset all the values and increase the round.
		...
		// Check to see if the AI won the round.
